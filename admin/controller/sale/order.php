...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
        ...
        if (isset($this->request->get['filter_store_location_id'])) {
            $filter_store_location_id = $this->request->get['filter_store_location_id'];
        } else {
        ...
        if (isset($this->request->get['filter_store_location_id'])) {
            $url .= '&filter_store_location_id=' . $this->request->get['filter_store_location_id'];
        }
	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$code=$this->db->query("SELECT * FROM `product_to_code` WHERE product_id='".$product['product_id']."'");
	...
	public function invoice() {
		...
		$abc=$this->db->query("SELECT * FROM `box_subscription` WHERE customer_id='".$order_info['customer_id']."'");
		if($abc->num_rows!='0'&& $abc->row['payment_status']!='1'){
			$data['box_pay'] = '1';
		}
		$dona=$this->db->query("SELECT * FROM `donation_covid` WHERE order_id=$order_id");
		if($dona->num_rows!='0'){
			$data['dona'] = '1';
		...
		$inv=$this->db->query("SELECT * FROM `oc_order` WHERE order_id=$order_id");
		if($inv->row['invoice_prefix']=="INV-2078-00"){
			$inv_no=$this->db->query("SELECT * FROM `2078_order_id` WHERE order_id=$order_id");
