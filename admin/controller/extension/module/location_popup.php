...
class ControllerExtensionModuleLocationPopup extends Controller {
	...
	public function index() {		
		if (isset($this->request->get['module_id'])) {
			...
			$this->model_setting_module->addModule('location_popup', self::DEFAULT_MODULE_SETTINGS); /* Becouse modules are being deleted by extension name */
			$this->response->redirect($this->url->link('extension/module/location_popup','&user_token='.$this->session->data['user_token'].'&module_id='.$this->db->getLastId()));
		}
	}
	...
	protected function configure($module_id) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/location_popup', 'user_token=' . $this->session->data['user_token'], true)
		);
		...
		$this->response->setOutput($this->load->view('extension/module/location_popup', $data));
	}

	public function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/location_popup')) {
	...
	public function install() {
		...
		$this->model_setting_setting->editSetting('module_alocation_popup', ['module_location_popup_status'=>1]);
		$this->model_setting_module->addModule('location_popup', self::DEFAULT_MODULE_SETTINGS); 
	}
	
	public function uninstall() {
		$this->load->model('setting/setting');
		$this->model_setting_setting->deleteSetting('module_location_popup');
	}
}
