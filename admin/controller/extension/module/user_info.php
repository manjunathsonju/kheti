...
class ControllerExtensionModuleUserInfo extends Controller {
	...
	const DEFAULT_MODULE_SETTINGS = [
		'name' => 'User Info',
		'message' => 'Activate the module ?',
		'age' => '8',
		'redirect_url' => 'http://khetifood.com',
	...
	public function index() {		
		if (isset($this->request->get['module_id'])) {
			...
			$this->model_setting_module->addModule('user_info', self::DEFAULT_MODULE_SETTINGS); /* Becouse modules are being deleted by extension name */
			$this->response->redirect($this->url->link('extension/module/user_info','&user_token='.$this->session->data['user_token'].'&module_id='.$this->db->getLastId()));
		}
	}
	...
	protected function configure($module_id) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/user_info', 'user_token=' . $this->session->data['user_token'], true)
		);
		...
		$this->response->setOutput($this->load->view('extension/module/user_info', $data));
	}

	public function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/user_info')) {
	...
	public function install() {
		...
		$this->model_setting_setting->editSetting('module_user_info', ['module_user_info_status'=>1]);
		$this->model_setting_module->addModule('user_info', self::DEFAULT_MODULE_SETTINGS); 
	}
	
	public function uninstall() {
		$this->load->model('setting/setting');
		$this->model_setting_setting->deleteSetting('module_user_info');
	}
}
