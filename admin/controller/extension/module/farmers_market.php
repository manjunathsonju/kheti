...
class ControllerExtensionModuleFarmersMarket extends Controller {
	...
	const DEFAULT_MODULE_SETTINGS = [
		'name' => 'Farmers Market',
		'message' => 'Farmers sell their local produces.',
		'age' => 20,
		'redirect_url' => 'https://khetifood.com',
	...
	public function index() {		
		if (isset($this->request->get['module_id'])) {
			...
			$this->model_setting_module->addModule('farmers_market', self::DEFAULT_MODULE_SETTINGS); /* Becouse modules are being deleted by extension name */
			$this->response->redirect($this->url->link('extension/module/farmers_market','&user_token='.$this->session->data['user_token'].'&module_id='.$this->db->getLastId()));
		}
	}
	...
	protected function configure($module_id) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/farmers_market', 'user_token=' . $this->session->data['user_token'], true)
		);
		...
		$this->response->setOutput($this->load->view('extension/module/farmers_market', $data));
	}

	public function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/farmers_market')) {
	...
	public function install() {
		...
		$this->model_setting_setting->editSetting('module_farmers_market', ['module_farmers_market_status'=>1]);
		$this->model_setting_module->addModule('farmers_market', self::DEFAULT_MODULE_SETTINGS); 
	}
	
	public function uninstall() {
		$this->load->model('setting/setting');
		$this->model_setting_setting->deleteSetting('module_farmers_market');
	}
}
