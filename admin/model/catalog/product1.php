...
class ModelCatalogProduct extends Model
{
        ...
        foreach ($data['product_description'] as $language_id => $value) {
            $this->db->query(
                'INSERT INTO ' .
                    ...
                    "', transparency = '" .
                    $this->db->escape($value['transparency']) .
                    "',nutritional_value='" .
                    $this->db->escape($value['nutritional_value']) .
                    "', tag = '" .
    ...
    public function editProduct($product_id, $data)
    {
        ...
        // $this->db->query("UPDATE `oc_product_description` SET taxable='".$data['taxable']."' WHERE product_id = '" . (int)$product_id . "'");
    ...
    {
        //		$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store ps ON (p.product_id = ps.product_id) JOIN " . DB_PREFIX . "product_description pd ON (ps.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
        $sql =
            'SELECT * FROM ' .
            DB_PREFIX .
            'product p LEFT JOIN ' .
            DB_PREFIX .
            'product_to_store ps ON (p.product_id = ps.product_id) JOIN ' .
            DB_PREFIX .
            'product_description pd ON (ps.product_id = pd.product_id) JOIN ' .
            DB_PREFIX .
            "product_to_category ptc ON (p.product_id = ptc.product_id) WHERE pd.language_id = '" .
            (int) $this->config->get('config_language_id') .
            "'";

        if (
            isset($data['filter_store']) &&
            (!empty($data['filter_store']) or $data['filter_store'] == '0')
        ) {
            $sql .=
                ' AND ps.store_id = ' .
                $this->db->escape($data['filter_store']);
        }
    ...
    public function getProductDescriptions($product_id)
    {
        ...
        foreach ($query->rows as $result) {
            $product_description_data[$result['language_id']] = [
                ...
                'nutritional_value' => $result['nutritional_value'],
