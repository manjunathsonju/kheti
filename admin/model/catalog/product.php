...
class ModelCatalogProduct extends Model
{
    ...
    public function getProductsByCategoryIdP($product_id)
    {
        $query = $this->db->query(
            'SELECT p.product_id,cat.name FROM ' .
                ...
                'product_description pd ON (p.product_id = pd.product_id) LEFT JOIN ' .
                DB_PREFIX .
                "product_to_category p2c ON (p.product_id = p2c.product_id) LEFT JOIN oc_category_description cat on (p2c.category_id = cat.category_id) WHERE pd.language_id = '1' AND p2c.product_id = '" .
                (int) $product_id .
                "' ORDER BY pd.name ASC"
        );
    ...
    public function getProductCategorygroup($product_id)
    {
        $product_category_data = [];

        $query = $this->db->query(
            'SELECT t1.product_id,t1.category_id,t2.name FROM oc_product_to_category t1 left join oc_category_description t2 on t1.category_id=t2.category_id WHERE product_id = ' .
                "'1004'" .
