 ...
 * @version 1.0
 */
class ModelExtensionModulePavFlashsale extends Model {
	 ...
	 */
	//#1 start edit code
	public function getProducts($data = array(), $flag = true) {

		$this->load->model('catalog/product');

		$filter = " AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ";
		if($data['date_type'] == "date") { //today
			$filter = " AND (ps.date_end <= '{$data['end_date']}') ";
		} elseif($data['date_type'] == 8888) { //available
			$filter = " AND ps.date_end > '{$data['end_date']}' ";
		} elseif($data['date_type'] == "nowtof") {//available
			$filter = " AND ps.date_start >=now() ";
		} else { //pass

			// $filter = " AND ps.date_start >= '{$data['start_date']}' AND ps.date_end < '{$data['end_date']}' ";
			$filter = " AND (";
			$filter .= "(ps.date_start >= '{$data['start_date']}' AND ps.date_end < '{$data['end_date']}')";
			$filter .= " OR (ps.date_end > '{$data['start_date']}' AND ps.date_end < '{$data['end_date']}')";
			$filter .= ")";
		}
		$join = "";
