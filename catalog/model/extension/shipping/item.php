	...
	function getQuote($address) {
		...
		if ($status) {
			...
			$quote_data['item'] = array(
				...
				'cost'         => $this->config->get('shipping_item_cost') * (int)$items,
				'tax_class_id' => $this->config->get('shipping_item_tax_class_id'),
				'text'         => $this->currency->format($this->tax->calculate($this->config->get('shipping_item_cost') * (int)$items, $this->config->get('shipping_item_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
			);
