...
class ModelExtensionTotalCoupon extends Model {
	...
	public function getTotalCouponHistoriesByCustomerId1($coupon, $customer_id) {
		// $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "coupon_history` ch LEFT JOIN `" . DB_PREFIX . "coupon` c ON (ch.coupon_id = c.coupon_id) JOIN oc_order o ON (o.order_id=ch.order_id) WHERE o.order_status_id IN (1,2,5) AND c.code = '" . $this->db->escape($coupon) . "' AND ch.customer_id = '" . (int)$customer_id . "'");
	...
	public function getCoupon1($code,$customer_id) {
		$status = true;
		$coupon_exist = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code = '" . $this->db->escape($code)."'");
		...
		if ($coupon_query->num_rows) {
			...
			if ( $customer_id) {
				$customer_total = $this->getTotalCouponHistoriesByCustomerId1($code, $customer_id);
		...
		if ($status) {
			return array(
				...
				'name'          => $coupon_query->row['name'],
				'type'          => $coupon_query->row['type'],
				'discount'      => (float)$coupon_query->row['discount'],
				'shipping'      => $coupon_query->row['shipping'],
				'total'         => $coupon_query->row['total'],
				'product'       => $product_data,
				'date_start'    => $coupon_query->row['date_start'],
