...
class ModelExtensionPurpletreeMultivendorVendor extends Model{
	...
	public function addSeller($customer_id,$store_name,$filename = '', $farmer_market, $province, $district, $palika, $ward, $logistics, $taxation_type, $company_reg_no, $ips_connect_id){
		$this->db->query("INSERT into " . DB_PREFIX . "purpletree_vendor_stores SET seller_id ='".(int)$customer_id."', store_name='".$this->db->escape(trim($store_name))."', farmer_market='".$this->db->escape(trim($farmer_market)). "', province='".$this->db->escape(trim($province)) .
		"', district='".$this->db->escape(trim($district)) .
		"', palika='".$this->db->escape(trim($palika)) .
		"', ward='".$this->db->escape(trim($ward)) .
		"', logistics='".$this->db->escape(trim($logistics)) .
		"', taxation_type='".$this->db->escape(trim($taxation_type)) .
		"', company_reg_no='".$this->db->escape(trim($company_reg_no)) .
		"', ips_connect_id='".$this->db->escape(trim($ips_connect_id)) .
		"', multi_store_id='".(int)($this->config->get('config_store_id'))."',store_status='".(int)(!$this->config->get('module_purpletree_multivendor_seller_approval'))."', store_created_at= NOW(), store_updated_at= NOW()");
