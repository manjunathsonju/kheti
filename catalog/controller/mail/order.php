...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		foreach ($results as $result) {
            ...
            $href = 'khetifood.com/index.php?route=product/product&product_id='.$result['product_id'].'&social=mail';
            if ((float)$result['special']) {
                $special = $this->currency->format($this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
		...
		// products end 
		//send donation info
		$sql="SELECT * FROM `donation_covid` WHERE order_id='".$order_info[order_id]."'";

		$donaa=$this->db->query($sql);
		if($donaa->num_rows!='0'){
			$data['donation'] = 1;
			$data['amount_donation'] = $donaa->row['amount'];
		...
		$ru = getrusage();
		$string1 =  "This process used " . rutime($ru, $rustart, "utime") .
			" ms for its computations\n";
		$string2 =  "It spent " . rutime($ru, $rustart, "stime") .
			" ms in system calls\n";
			
		$fp = fopen('error_log_3908.txt', 'w');
		fwrite($fp, $string1);
		fwrite($fp, $string2);
		fclose($fp);
	}
