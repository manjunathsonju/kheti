...
class ControllerApiB2bDashlogin extends Controller
{
    ...
    {
        $hash = $this->db->query(
            "SELECT * FROM b2b_users where firstname='" .
        ...
        if ($hash) {
            ...
            if ($decryption == $this->request->get['password']) {
                $token = token(9);
                $this->db->query(
                    "UPDATE b2b_users SET token='$token' WHERE user_id='" .
                        $id .
                        "'"
                );
                $dt = strtotime(now);
                ...
                $sql =
                    "INSERT INTO `b2b_activity_log_dashboard` (activity_type,user,change_id,date) VALUES ('login','" .
                    $id .
                    "','0','" .
                    $dtm .
                    "')";
                ...
                header(
                    'location: https://khetifood.com/index.php?route=b2b/home&id=' .
                        $id .
                        '&tkn=' .
                        $token
                );
    ...
    public function adduser()
    {
        if ($this->request->get['pass'] == 'rajiv') {
            ...
            $this->db->query(
                "INSERT INTO b2b_users (firstname, lastname, password) values ('" .
                    $this->request->get['firstname'] .
                    "','" .
