<?php
class ControllerApiKhetifarmhome extends Controller {
    public function index(){  
        if($this->request->post['key']=='!3t3kf@rmh0m3'){
			...
			$this->load->model('kheti/components'); 
				...
				$json['components']=array(
					"1"=> array(
						'component_id' => 5,
						'component_name'=> 'kheti',
						'content' => array('image'=>'https://store.kheti.farm/image/cache/catalog/Agritools-883x355.jpeg'),'href'=>'','type'=>''),
		
					"2"=> array(
						'component_id' => 6,
						'component_name'=> 'Baked with love',
						'content' => array('image'=>'https://store.kheti.farm/image/cache/catalog/today-883x355.jpeg'),'href'=>'','type'=>''),
		
					"3"=> array(
						'component_id' => 7,
						'component_name'=> 'Kaushi Kheti',
						'content' => array('image'=>'https://khetifood.com/image/test/kausi.jpeg'),'href'=>'','type'=>''),
					

					"4"=> array(
						'component_id' => 7,
						'component_name'=> 'Dry Fruits',
						'content' => array('image'=>'https://store.kheti.farm/image/cache/catalog/whykheti-883x355.jpeg'),'href'=>'','type'=>''),
		
		
					"5"=> array(
						'component_id' => 7,
						'component_name'=> 'Kheti Subscription Box',
						// 'content' => array('image'=>'https://khetifood.com/image/test/pic2.jpeg'),'href'=>'221','type'=>'category'),
						'content' => array('image'=>'https://khetifood.com/image/test/kehtibox.jpeg'),'href'=>'','type'=>''),
					...
					"7"=> array(
						'component_id' => 1,
						'component_name'=> 'Vegan Dairy Nepal',
						'content' => array('image'=>'https://store.kheti.farm/image/cache/catalog/biubijan-883x355.jpeg'),'href'=>'','type'=>''),
		
					"8"=> array(
						'component_id' => 2,
						'component_name'=> 'Shop by category',
						'content' => $this->model_kheti_products->getcategories($location,0)),
		
					"9"=> array(
						'component_id' => 3,
						'component_name'=> 'slider',
						'content' => $this->model_kheti_components->index($location,0)),
							
		
					"10"=> array(
						'component_id' => 4,
						'component_name'=> 'products',
						'content' => array($this->model_kheti_products->categoryjson(60,$location),$this->model_kheti_products->categoryjson(151,$location),$this->model_kheti_products->categoryjson(139,$location),$this->model_kheti_products->categoryjson(257,$location),$this->model_kheti_products->categoryjson(215,$location))),
					...
					"15"=> array(
						...
						// 'content' => $this->model_kheti_products->getbestselling($location,0))	
                        'content' => NULL),
	...
	public function getkhetipointsfmt(){ 
		$data = json_decode(file_get_contents('php://input'), true); 
		// $json['contact']= $data['contact'];
		// $json['key']= $data['key'];

        if($data['key']=='po!nt5f@rm'){
			
		
		$sql="SELECT * FROM `oc_customer` WHERE telephone='".$data['contact']."'";
		$customer_info=$this->db->query($sql);

		if($customer_info->row['customer_id']){		
		
				$this->load->model('account/reward');
				$json['kheti_points']= $this->model_account_reward->getTotalPoints1($customer_info->row['customer_id']);				
	...
	public function orderssummary(){  
		$sql="SELECT * FROM `oc_customer` WHERE store_id=0 AND customer_id='".$this->request->post['customer_id']."'";
		$customer_info=$this->db->query($sql);

		if($this->request->post['customer_id']){
			
				$sql="SELECT COUNT(*) AS num_of_orders, SUM(total) as total FROM `oc_order` WHERE customer_id = '".$this->request->post['customer_id']."' AND order_status_id IN (1,2,5) AND store_id = '0'";
				$order_num=$this->db->query($sql);

				$json['number_of_orders']=$order_num->row['num_of_orders'];
				if($order_num->row['total']){
					$json['total_transaction_amount']=$order_num->row['total'];

				}else{
					$json['total_transaction_amount']='0';

				}
				$this->load->model('account/reward');

				$json['kheti_points']= $this->model_account_reward->getTotalPoints1($this->request->post['customer_id']);
	...
	public function footer(){ 
		...
		$json['contacts']=array(
            'contact_ktm' => 'Kupondole - 01, Lalitpur 9802393160',
	...
	public function getallzones(){ 
		...
		$sql="SELECT zone_id,name FROM `oc_zone` WHERE country_id=149 AND status=1";
		$zon=$this->db->query($sql);

		foreach($zon->rows as $result){
	...
	public function fmtpasswordcheck(){  
		$data = json_decode(file_get_contents('php://input'), true);// Converts it into a PHP object 

		$sql="SELECT * FROM `oc_customer` WHERE store_id=0 AND telephone='".$this->db->escape($data['number'])."'";
		$customer_info=$this->db->query($sql);

		if($customer_info->row['fmt_pwd']){
			$json['success']= 1;
...
// "key":"!3t3kf@rmr3g1ater",
// "number": "9860488334",
// "password":"rojinahengaju",

	$data = json_decode(file_get_contents('php://input'), true); 

	if($data['key']=='!3t3kf@rmr3g1ater'){
		$sql="SELECT * FROM `oc_customer` WHERE telephone='".$this->db->escape($data['number'])."'";
		$customer_info=$this->db->query($sql);

		if($customer_info->row['telephone']){			
			// $json['message']= 'login'; 
			if($customer_info->row['fmt_pwd']==$data['password']){
				$json['success']= 1;
				
			}else{
				$sql="UPDATE `oc_customer` SET fmt_pwd='".$data['password']."' WHERE `telephone`='".$data['number']."'";
				$this->db->query($sql);
			...
			$sql="INSERT INTO `customer_login_app` (customer_id,ip,device_info,date_added,status) VALUES ('".$customer_info->row['customer_id']."','".$data['ip']."','".$this->db->escape($data['device_info'])."',NOW(),'1')";
				$this->db->query($sql);
			...
			$customer_id_email = $this->model_account_customer->getCustomerByEmail($data['email']);
			$customer_id_num = $this->model_account_customer->getCustomerByPhone($data['number']);
			if($customer_id_email!= NULL){
				$email=$data['number'].'@yopmail.com';

			}else{
				$email=$data['email'];
			}
			if($customer_id_num== NULL){
                $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . '5' . "', store_id = '" . '0' . "', language_id = '" . '1' . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname'])  . "', email = '" . $this->db->escape($email) . "', telephone = '" . $this->db->escape($data['number']) . "', custom_field = '" .''. "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . '1' . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . '1' . "', date_added = NOW(), fmt_pwd='".$data['password']."'");
    
                $sql="SELECT * FROM `oc_customer` WHERE email='".$this->db->escape($email)."'";
                $customer_info=$this->db->query($sql);
                $token=token(10);
                $json['token']= $token;
                $sql="UPDATE `oc_customer` SET token_app='".$token."' WHERE customer_id='".$customer_info->row['customer_id']."'";
			...
			//code to manage post sessions
			if(($data['cart_token'])&&($data['cart_token']!='empty')){
				$sessioninformation= $this->findsession($customer_info->row['customer_id']);
				if($sessioninformation){
					$json['sessioninformation']= $sessioninformation;


				$sql="SELECT * FROM `oc_cart` WHERE session_id='".$data['cart_token']."'";
				  $products=$this->db->query($sql);
					...
					if($products_exist->row['product_id']){
						...
						$this->db->query($sql);
						$sql="DELETE FROM `oc_cart` WHERE session_id='".$data['cart_token']."' AND product_id='".$result['product_id']."'";
						$this->db->query($sql);

					}else{
						$sql="UPDATE `oc_cart` SET session_id='".$sessioninformation."' WHERE session_id='".$data['cart_token']."'";
						$this->db->query($sql);
				...
				$sql="UPDATE `oc_cart` SET customer_id='".$customer_info->row['customer_id']."' WHERE session_id='".$data['cart_token']."'";
				$this->db->query($sql);
				$json['session_id']= $data['cart_token'];
				}
	...
	public function addtocart(){  
        if($this->request->post['key']=='@d!3t3kf@mc@rt'){
	...
	public function getcartproducts(){  
        if($this->request->post['key']=='g3t!3t3kf@rmc@rt'){
            ...
            if(($session)&&($session!=NULL)){
                $sql="SELECT c.* FROM `oc_cart` c JOIN oc_product p ON (p.product_id=c.product_id) JOIN oc_product_to_store ps ON (ps.product_id=p.product_id) WHERE p.status=1 AND c.session_id='".$session."' AND ps.store_id=0";
                $products=$this->db->query($sql);
				$subtotal=0;
                ...
                foreach ($products->rows as $result){
					...
					$product_info = $this->model_catalog_product->getProduct1($result['product_id']);					
				...
				$json['recommended_products']=$this->model_kheti_products->getprodcuts1(59);
	...
	public function editcartproducts(){  
        if($this->request->post['key']=='3!t!3t3kfrmc@rt'){
	...
	public function removecartproduct(){  
		if($this->request->post['key']=='d3l3t3f@mc@rt'){
			$sql="SELECT * FROM `oc_cart` WHERE session_id='".$this->request->post['session_id']."'";
			$session_status=$this->db->query($sql);
	...
	public function valuedfarmers(){   
			$this->load->model('kheti/components');
			$json['farmers']=$this->model_kheti_components->valuedfarmers();
		$this->response->addHeader('Content-Type: application/json');
		$this->response->addHeader('Access-Control-Allow-Origin: *');
		$this->response->addHeader('Access-Control-Allow-Headers: *');
		$this->response->addHeader('Access-Control-Allow-Methods: POST, GET, OPTIONS, DELETE, PUT');
	...
	public function testimonials(){   
		$this->load->model('kheti/components');
		$json['testimonials']=$this->model_kheti_components->testimonials();
	$this->response->addHeader('Content-Type: application/json');
	$this->response->addHeader('Access-Control-Allow-Origin: *');
	$this->response->addHeader('Access-Control-Allow-Headers: *');
	$this->response->addHeader('Access-Control-Allow-Methods: POST, GET, OPTIONS, DELETE, PUT');
...
public function insertAddress(){   
	...
	public function gotocheckout(){ 
		$this->load->model('extension/total/coupon');
		   $login= $this->checklogin($this->request->post['customer_id'],$this->request->post['token']);
			if($login){
				$sql="SELECT c.* FROM `oc_cart` c JOIN oc_product p ON (p.product_id=c.product_id) JOIN oc_product_to_store ps ON (ps.product_id=p.product_id) WHERE p.status=1 AND c.session_id='".$this->request->post['session_id']."' AND ps.store_id=0";
		...
		public function confirm(){   
			$sql="SELECT * FROM `oc_order` WHERE order_id='".$this->request->post['order_id']."' AND order_status_id=0";
	...
	public function getslider(){ 
		$this->load->model('kheti/components'); 
		$json['slider'] =$this->model_kheti_components->index('ktm',0);
		$this->response->addHeader('Content-Type: application/json');
		$this->response->addHeader('Access-Control-Allow-Origin: *');
		$this->response->addHeader('Access-Control-Allow-Headers: *');
		$this->response->addHeader('Access-Control-Allow-Methods: POST, GET, OPTIONS, DELETE, PUT');
	...
	public function homeproducts(){ 
		$this->load->model('kheti/products'); 
		$json['products'] =	array($this->model_kheti_products->categoryjsonk(60,'ktm'),$this->model_kheti_products->categoryjsonk(148,'ktm'));
		$this->response->addHeader('Content-Type: application/json');
		$this->response->addHeader('Access-Control-Allow-Origin: *');
		$this->response->addHeader('Access-Control-Allow-Headers: *');
		$this->response->addHeader('Access-Control-Allow-Methods: POST, GET, OPTIONS, DELETE, PUT');
	...
	public function getCategories(){ 
		$this->load->model('kheti/products'); 
		$json['categories'] =$this->model_kheti_products->getcategories('ktm',0);
		$this->response->addHeader('Content-Type: application/json');
		$this->response->addHeader('Access-Control-Allow-Origin: *');
		$this->response->addHeader('Access-Control-Allow-Headers: *');
		$this->response->addHeader('Access-Control-Allow-Methods: POST, GET, OPTIONS, DELETE, PUT');
	...
	public function getBanners(){ 
		...
		// $json['categories'] =$this->model_kheti_products->getcategories('ktm',0);
		$json['banners'] = array(
			array('image'=>'https://store.kheti.farm/image/cache/catalog/Agritools-883x355.jpeg','href'=>'','type'=>''),
			array('image'=>'https://store.kheti.farm/image/cache/catalog/today-883x355.jpeg','href'=>'','type'=>''),
			array('image'=>'https://khetifood.com/image/test/kausi.jpeg','href'=>'','type'=>''),
			array('image'=>'https://store.kheti.farm/image/cache/catalog/whykheti-883x355.jpeg','href'=>'','type'=>''),
			array('image'=>'https://khetifood.com/image/test/kehtibox.jpeg','href'=>'','type'=>''),
			array('image'=>'https://store.kheti.farm/image/cache/catalog/biubijan-883x355.jpeg','href'=>'','type'=>''),
	...
	public function popularSearches(){ 
		$this->load->model('kheti/components'); 
		$json['keywords'] =$this->model_kheti_components->popularSearches(0);
		$this->response->addHeader('Content-Type: application/json');
		$this->response->addHeader('Access-Control-Allow-Origin: *');
		$this->response->addHeader('Access-Control-Allow-Headers: *');
		$this->response->addHeader('Access-Control-Allow-Methods: POST, GET, OPTIONS, DELETE, PUT');
	...
	// 	$customer_info=$this->db->query($sql);
	// 	$data['reset'] = 'https://khetifood.com/index.php?route=account/reset&code='.$customer_info->row['code'];
	...
	public function search(){   
		...
		// $categories_1 = $this->model_catalog_category->getCategories1(0);

		// foreach ($categories_1 as $category_1) {
		// 	$level_2_data = array();

		// 	$categories_2 = $this->model_catalog_category->getCategories1($category_1['category_id']);

		// 	foreach ($categories_2 as $category_2) {
		// 		$level_3_data = array();

		// 		$categories_3 = $this->model_catalog_category->getCategories1($category_2['category_id']);
		...
		if (isset($this->request->post['search']) || isset($this->request->get['tag'])) {
			...
			foreach ($results as $result) {
				...
				//$category_name = '';
				// $category = $this->model_catalog_product->getCategories1($result['product_id']);
				// if ($category){
				// 	$category_array = $this->model_catalog_category->getCategory1($category[0]['category_id']);
...
<div class="content-item">
			...
			<table class="table  table-striped  table-bordered border-primary " style="width: 375px; margin: 10px 10px;">
				...
				<tbody>                                 
						...
						<tr>
						  <td><p class="par">Order id:</p>'.$data['order_id'].'
						...
						<table class="table  table-striped table-bordered border-primary" style="width: 280px; margin: 10px 10px;">
				<thead class="table-light ">
							<tr>
...
public function getwishlist(){ 
	...
	public function profile(){
		if(($this->request->post['customer_id'])&&($this->request->post['token'])){
			...
			if($customer_info->row['token_app']==$this->request->post['token']){
				...
				$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, os.name as status, o.date_added, o.total, o.currency_code, o.currency_value FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE o.customer_id = '" . $this->request->post['customer_id']. "' AND o.order_status_id > '0' AND o.store_id = '0' AND os.language_id = '1' ORDER BY o.order_id DESC");
	...
	public function orderProductsHistory(){
		$data = json_decode(file_get_contents('php://input'), true);
		$products = []; 


		if(($data['customer_id'])&&($data['key']=="h1sTrYpr0fmT")){

			$query = $this->db->query("SELECT o.order_id,os.name as status, o.date_added, o.total, o.currency_code, o.currency_value FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE o.customer_id = '" . $data['customer_id']. "' AND o.order_status_id > '0' AND o.store_id = '0' AND os.language_id = '1' ORDER BY o.order_id DESC");
			foreach ($query->rows as $result) {
		$this->load->model('account/order');
		$query = $this->db->query("SELECT * FROM oc_order_product WHERE order_id = '" . $result['order_id']. "'");
		foreach ($query->rows as $productresult) {
	...
	public function orderHistorydetails(){
		...
		if($data['key']=="h1sTrY0rderfmT"){
			$query1 = $this->db->query("SELECT COUNT(*) AS num FROM oc_order WHERE store_id=0 AND order_status_id IN (2,5)");

			$query = $this->db->query("SELECT SUM(total) as total FROM oc_order WHERE store_id=0 AND order_status_id IN (2,5)");
			$total=(int)$query->row['total']; 
			$num=(int)$query1->row['num']; 
