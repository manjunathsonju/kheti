<?php
Class ControllerApiCompiledTotalnew extends Controller{
    public function index(){      
      // if($_GET['pass']=="2020kheti@DV") {
        $productidarr=$this->db->query("SELECT DISTINCT op.product_id from oc_order_product op JOIN oc_product p ON (op.product_id=p.product_id) where op.order_id BETWEEN ".$_GET['from'] ." AND ".$_GET['to']." AND p.location='ktm' ORDER BY op.model ");
