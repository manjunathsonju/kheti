...
class ControllerExtensionAccountPurpletreeMultivendorSellerstore extends Controller{
	...
	public function index(){
		...
		if(VERSION=='3.1.0.0_b'){
			$this->document->addScript('catalog/view/javascript/purpletree_style_bs4.js');	
		}else{
 ...
 $this->document->addStyle('catalog/view/javascript/purpletree/codemirror/theme/monokai.csss'); 
 ...
 $this->document->addScript('catalog/view/javascript/purpletree/summernote/opencart.js'); 
		...
		$data['seller_store_id']=$this->request->get['seller_store_id'];
	...
	public function becomeseller(){
		...
		if (isset($this->request->post['farmer_market'])) { 
			...
			$data['farmer_market'] = $seller_info['farmer_market'];
		} else { 
			$data['farmer_market'] = '';
		}
...
////End Menu
		...
		if($seller_products){
			...
			foreach($seller_products as $seller_product){
				...
				$category_name = '';
				$category = $this->model_catalog_product->getCategories($seller_product['product_id']);
				if ($category){
				...
				$data['seller_products'][] = array(
					...
					'location' => $seller_product['location'],
