...
class ControllerExtensionAccountPurpletreeMultivendorSellercontact extends Controller {
	...
	public function sellercontactlist(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatemessage()) {
			...
			$ptsmv_current_page='';
			$message = 'Seller Name - '.$selllleeerr['firstname'].' '. $selllleeerr['lastname'].'<br>';
			$message .= 'Seller Email - '.$selllleeerr['email'].'<br>';
			$message .= 'Message - '.$this->request->post['customer_message'];			
			$mail = new Mail();
			...
			$mail->setSender(html_entity_decode($selllleeerr['firstname'], ENT_QUOTES, 'UTF-8'));
			$mail->setSubject(html_entity_decode(sprintf($this->language->get('email_subject'), $selllleeerr['firstname']), ENT_QUOTES, 'UTF-8'));
	...
	if (!$this->customer->isLogged()) {
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(isset($this->session->data['ptsmv_current_page']) && isset($this->session->data['ptsmv_current_page_name'])) {
				...
				$referrerUrl = '<br> Referrer Page - '.'<a href="'.SERVER_PROTOCOL.$producturl.'" target="_blank">'.$pagename.'</a>';
			}
			$dataa = array(
					'customer_id' 	 => $customerid,
					'seller_id'		 => $seller_id,
			...
			$this->model_extension_purpletree_multivendor_sellercontact->addContact($dataa);
			$ptsmv_current_page='';
			$message = 'Customer Name - '.$this->request->post['customer_name'].'<br>';
			$message .= 'Customer Email - '.$this->request->post['customer_email'].'<br>';
			$message .= 'Message - '.$this->request->post['customer_message'];
			if(isset($this->session->data['ptsmv_current_page'])) {
				$message .= '<br>Product URL - '.$this->session->data['ptsmv_current_page'];
			}
			if($referrerUrl != '') {
				$message .= $referrerUrl;
			}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatemessage()) {
			...
			$this->model_extension_purpletree_multivendor_sellercontact->addContact($dataa);
			$ptsmv_current_page='';
			$message = 'Customer Name - '.$customerrr['firstname'].' '.$customerrr['lastname'].'<br>';
			$message .= 'Customer Email - '.$customerrr['email'].'<br>';
			$message .= 'Message - '.$this->request->post['customer_message'];
			if(isset($this->session->data['ptsmv_current_page'])) {
				$message .= '<br>Product URL - '.$this->session->data['ptsmv_current_page'];
			}
			...
			$mail->setTo($sellerr['email']);
			$mail->setFrom($this->config->get('config_email'));
			$mail->setSender(html_entity_decode($customerrr['firstname'], ENT_QUOTES, 'UTF-8'));
			$mail->setSubject(html_entity_decode(sprintf($this->language->get('email_subject'), $customerrr['firstname']), ENT_QUOTES, 'UTF-8'));
			$mail->setHtml($message);
			$mail->send();
