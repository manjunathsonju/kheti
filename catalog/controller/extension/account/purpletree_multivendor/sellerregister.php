...
class ControllerExtensionAccountPurpletreeMultivendorSellerregister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$file = "";
					...
					$farmer_market = trim($this->request->post['farmer_market']);
					$province = trim($this->request->post['province']);
					$district = trim($this->request->post['district']);
					$palika = trim($this->request->post['palika']);
					$ward = trim($this->request->post['ward']);
					$logistics = trim($this->request->post['logistics']);
					$taxation_type = trim($this->request->post['taxation_type']);
					$company_reg_no = trim($this->request->post['company_reg_no']);
					$ips_connect_id = trim($this->request->post['ips_connect_id']);
					
					$this->load->model('extension/purpletree_multivendor/vendor');
					$file = '';
					$seller_id = $this->model_extension_purpletree_multivendor_vendor->addSeller($customer_id,$store_name ,$file, $farmer_market, $province, $district, $palika, $ward, $logistics, $taxation_type, $company_reg_no, $ips_connect_id);
	 ...
	 /* if (isset($this->error['store_email'])) {
		...
		/*farmer market*/
		if (isset($this->request->post['farmer_market'])) {
				$data['farmer_market'] = $this->request->post['farmer_market'];
		} else {
			$data['farmer_market'] = '';
		}
		
		/*province*/
		if (isset($this->request->post['province'])) {
		...
		/*palika*/
		if (isset($this->request->post['palika'])) {
				$data['palika'] = $this->request->post['palika'];
		} else {
			$data['palika'] = '';
		}
		...
		if (isset($this->request->post['ward'])) {
				$data['ward'] = $this->request->post['ward'];
		} else {
			$data['ward'] = '';
		}
		...
		/*taxation_type*/
		if (isset($this->request->post['taxation_type'])) {
				$data['taxation_type'] = $this->request->post['taxation_type'];
		} else {
			$data['taxation_type'] = '';
		}
		...
		/*ips_connect_id*/
		if (isset($this->request->post['ips_connect_id'])) {
				$data['ips_connect_id'] = $this->request->post['ips_connect_id'];
		} else {
			$data['ips_connect_id'] = '';
		}
	...
	/* 	if(!empty($this->request->post['store_email'])){
		...
		if ($this->model_account_customer->getTotalCustomersByMobile($this->request->post['telephone'])) {
			$this->error['warning'] = "Warning: Mobile number is already registered!";
		}
